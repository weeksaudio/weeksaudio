  ...
  list.innerHTML = tracks.map((t, i) => `
    <div class="track-row ${i === currentIndex ? 'active' : ''}" id="row-${i}" onclick="selectTrack(${i}, true)">
      ...
      <div class="track-right">
        ...
        <button class="track-buy-btn" onclick="event.stopPropagation(); buyTrack('${t.id}', '${escHtml(t.name)}', ${t.price}, '${t.audio_url}', '${t.contract_url || ''}')">BUY — $${Number(t.price).toFixed(2)}</button>
      </div>
    </div>
  `).join('');
...
// ── Stripe checkout ──
async function buyTrack(trackId, trackName, price, audioUrl, contractUrl) {
  ...
  try {
    const res = await fetch('/.netlify/functions/create-checkout', {
      ...
      body: JSON.stringify({ trackId, trackName, price, audioUrl, contractUrl }),
    });
...
function selectTrack(index, autoplay) {
  ...
  buyBtn.onclick = (e) => {
    e.preventDefault();
    buyTrack(t.id, t.name, t.price, t.audio_url, t.contract_url || '');
  };
