...
// iOS requires the audio element to have these attributes set
// and the AudioContext to be resumed on user interaction.
audio.setAttribute('playsinline', '');
audio.setAttribute('webkit-playsinline', '');

// Keep the audio session alive on iOS by registering with MediaSession API.
// iOS will terminate background audio if the MediaSession is not actively maintained.
function updateMediaSession(track) {
  ...
  // These handlers are required for iOS lock screen controls to appear,
  // which also signals to iOS that this is intentional background audio.
  ...
  list.innerHTML = tracks.map((t, i) => `
    <div class="track-row ${i === currentIndex ? 'active' : ''}" id="row-${i}" onclick="selectTrack(${i}, true)">
      ...
      <div class="track-right">
        ...
        <button class="track-buy-btn" onclick="event.stopPropagation(); buyTrack(${i})">BUY — $${Number(t.price).toFixed(2)}</button>
      </div>
    </div>
  `).join('');
...
// ── Stripe checkout ──
// Looks the track up by index rather than inlining its fields into the
// onclick string — avoids breaking (or being exploitable via) track names
// or URLs that contain quote characters.
async function buyTrack(index) {
  ...
  try {
    const res = await fetch('/.netlify/functions/create-checkout', {
      ...
      body: JSON.stringify({
        ...
        contractUrl: t.contract_url || '',
      }),
    });

    // If the server returned a non-OK status, surface the real error
    if (!res.ok) {
      let errMsg = 'Checkout unavailable. Please try again later.';
      try {
    ...
    // Show a user-friendly message in the button itself briefly
    btn.textContent = 'ERROR — TRY AGAIN';
    btn.style.borderColor = '#b03020';
...
function selectTrack(index, autoplay) {
  ...
  buyBtn.textContent = `BUY — $${Number(t.price).toFixed(2)}`;
...
// ── Playback ──
function startPlay() {
  // iOS: AudioContext must be created/resumed inside a user-gesture call chain.
  // We do it here because startPlay is always triggered by a tap (directly or
  // via canplaythrough which itself was set up inside a tap handler).
  if (!audioCtx) {
    try {
...
audio.addEventListener('timeupdate', () => {
  ...
  if (audio.duration) {
    ...
    // Keep MediaSession position state updated (helps iOS background mode)
